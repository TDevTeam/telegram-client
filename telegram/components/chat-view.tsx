...
export function ChatView() {
  ...
  useEffect(() => {
    // Check if the active chat has any unread messages with mentions
    if (activeChat && messages[activeChat.id]) {
      const newHasMentions = messages[activeChat.id].some(
        (message) => !message.isRead && message.content.includes(`@${activeAccount.username}`),
      )

      // If this chat belongs to the active account, update mentions status
      if (activeChat.accountId === activeAccount.id && newHasMentions !== hasMentions) {
        updateAccountMentions(activeAccount.id, newHasMentions)
        setHasMentions(newHasMentions)
      }
    }
  }, [activeChat, messages, activeAccount, hasMentions])

  const handleMarkAsRead = async (messageId) => {
    // Existing code...

    // After marking as read, check if there are any remaining unread mentions
    if (!activeChat) return
    const remainingUnreadMentions = messages[activeChat.id].some(
      (message) =>
        !message.isRead && message.id !== messageId && message.content.includes(`@${activeAccount.username}`),
    )

    // Update mentions status if needed
    if (!remainingUnreadMentions && hasMentions) {

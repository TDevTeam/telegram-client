...
export function AccountSidebar() {
  ...
  const handleSendCode = () => {
    // This would trigger a backend API call to send the verification code
    setShowVerificationInput(true)
  }

  const handleVerifyCode = () => {
    // This would trigger a backend API call to verify the code
    setShowAddAccountDialog(false)
    setShowVerificationInput(false)
    setPhoneNumber("")
    setVerificationCode("")
    // The backend would add the new account and stream the updated accounts list
  }
  ...
  const handleSaveAccountEdit = () => {
    // This would trigger a backend API call to update the account
    setShowEditAccountDialog(false)
    setEditingAccount(null)
  }

  const handleAccountClick = (account: (typeof accounts)[0]) => {
    // This would trigger a backend API call to switch the active account
              ...
              <Tooltip key={account.id}>
                <TooltipTrigger asChild>
                  ...
                  >
                    ...
                    {account.unreadCount > 0 && (
                      <Badge
                        className={`absolute -right-1 -top-1 h-5 w-5 rounded-full p-0 flex items-center justify-center ${
                          account.muted ? "bg-muted text-muted-foreground" : ""
                        }`}
                      >
                    ...
                    {account.hasMentions && (
                      <div className="absolute -right-1 bottom-0 h-3 w-3 rounded-full bg-red-500 border-2 border-background"></div>
                    )}
                  </Button>
                </TooltipTrigger>
                <TooltipContent side="right">
                  ...
                  {account.unreadCount > 0 && (
                    <p className="text-xs font-medium">
                      {account.unreadCount} unread {account.unreadCount === 1 ? "message" : "messages"}
                    </p>
                  )}
                  {account.hasMentions && <p className="text-xs text-red-500 font-medium">Mentions</p>}
